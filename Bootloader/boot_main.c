...
// ???????????????????????
#define APP_A_ADDR          0x08008000U
#define APP_B_ADDR          0x08020000U

// ??? APP ??????
static int IsAppValid(uint32_t app_addr);
// ???????? APP???????????????????
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
  // ??????????
  uint32_t upgrade_flag = *(__IO uint32_t *)BOOT_SLOT_FLAG_ADDR;
...
static int IsAppValid(uint32_t app_addr)
{
    ...
    if (sp < 0x20000000 || sp > 0x2000FFFF) return 0;
    if (reset_vector < 0x08000000 || reset_vector > 0x0803FFFF) return 0;
    return 1;
}

static void JumpToApp(uint32_t app_addr) {
    __set_MSP(*(volatile uint32_t*)app_addr);
    ((jump_func)(*(volatile uint32_t*)(app_addr + 4)))();
}
